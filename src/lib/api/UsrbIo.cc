...
int hf3fs_iovopen_device(struct hf3fs_iov *iov,
  ...
  XLOGF(ERR, "Device runtime not available, cannot open device iov for device {}", device_id);
  return -ENOTSUP;
}
#endif
...
int hf3fs_iovwrap_device(struct hf3fs_iov *iov,
  ...
  XLOGF(ERR, "Device runtime not available, cannot wrap device buffer for device {}", device_id);
  return -ENOTSUP;
}
#endif
